#ifndef _BACKPROJECTION_KERNEL_HALF_UM_16_H_
#define _BACKPROJECTION_KERNEL_HALF_UM_16_H_
 ...
 */
__global__ void backprojection_VIB_kernel_v0_16reg_half_UM(half *volume_g,int N_sLphi)
{
	...
	if ((xn_prime)*(xn_prime)+(yn_prime)*(yn_prime)<((xVolumePixelNb_GPU)/2)*((xVolumePixelNb_GPU)/2))
	{
		for (zn_prime=zn_start-zVolumeCenterPixel_GPU;zn_prime<zn_start-zVolumeCenterPixel_GPU+16;zn_prime+=16)
		{
			...
			vn+=A;
			valeur_dans_registre_3=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_4=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_5=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_6=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_7=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_8=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_9=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_10=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_11=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_12=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_13=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_14=tex2DLayered(sinogram_tex0,un,vn,phi_layer);

			vn+=A;
			valeur_dans_registre_15=tex2DLayered(sinogram_tex0,un,vn,phi_layer);
			...
			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_2*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_3*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_4*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_5*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_6*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_7*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_8*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_9*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_10*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_11*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_12*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_13*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_14*xVolumePixelSize_GPU);
			offset_volume+=a;

			volume_g[offset_volume]= __float2half_rn(valeur_dans_registre_15*xVolumePixelSize_GPU);
			offset_volume+=a;
		}
	}
