#ifndef _BACKPROJECTION_KERNEL_8_H_
#define _BACKPROJECTION_KERNEL_8_H_
 ...
 */
template <typename T>
__global__ void backprojection_VIB_kernel_v0_8reg(T *volume_g,int phi_start,int zn_start,int N_sLphi,int vn_start)
{
	...
	if ((xn_prime)*(xn_prime)+(yn_prime)*(yn_prime)<((xVolumePixelNb_GPU)/2)*((xVolumePixelNb_GPU)/2))
	{
		...
		for (zn_prime=zn_start-zVolumeCenterPixel_GPU;zn_prime<zn_start-zVolumeCenterPixel_GPU+16;zn_prime+=8)
		{
			...
			un=(-betaIOcylinderC_GPU[phi]*xVolumePixelSize_GPU*(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*xn_prime+alphaIOcylinderC_GPU[phi]*xVolumePixelSize_GPU*(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*yn_prime)*wn_inverse+uDetectorCenterPixel_GPU+0.5f;
			A=1.0/gammaIOcylinderC_GPU*wn_inverse;
			vn=A*zn_prime+vDetectorCenterPixel_GPU+0.5f;
			vn_tex=vn-vn_start;
