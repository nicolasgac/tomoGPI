...
#ifndef _CONVOLUTION3D_SEP_SHARED_KERNEL_HALF_H_
#define _CONVOLUTION3D_SEP_SHARED_KERNEL_HALF_H_

////////////////////////////////////////////////FIRST SLICES KERNELS/////////////////////////////////////////////////////////////
//-------------------------------------------------------------------------------------------------------------------------------
__global__ 	void  ConvKernel_h_shared_half	 (unsigned short* d_Output,unsigned short* d_Input,int down)
{
	...
	for (int pos = 	NUMBER_HALO_BLOCK ; pos < NUMBER_COMPUTED_BLOCK + NUMBER_HALO_BLOCK ; pos++)
	{
		...
		d_Output[pos*BLOCK_SIZE_H_X] = __float2half_rn(tmp_sum);
	}
#endif
...
//-------------------------------------------------------------------------------------------------------------------------------
__global__ 	void  ConvKernel_v_shared_half	 (unsigned short* d_Output,unsigned short* d_Input)
{
	// Shared memory declaration, including edge halo zones
	...
	for (int pos = 0 ; pos < NUMBER_HALO_BLOCK ; pos++)
	{
		if ( pixel_y >= -pos*BLOCK_SIZE_V_Y) //negative sign because we are outside block boudaries
			s_Data[threadIdx.y][threadIdx.x + pos*BLOCK_SIZE_V_Y] = __half2float(d_Input[pos*volume_offset2]) ;

		else
			s_Data[threadIdx.y][threadIdx.x + pos*BLOCK_SIZE_V_Y] = __half2float(0) ;
	}
	...
	for (int pos = 	NUMBER_HALO_BLOCK ; pos < NUMBER_COMPUTED_BLOCK + NUMBER_HALO_BLOCK ; pos++)
	{
		...
		d_Output[pos*volume_offset2] = __float2half_rn(tmp_sum);
	}
}
...
//-------------------------------------------------------------------------------------------------------------------------------
__global__ 	void  ConvKernel_p_shared	 (unsigned short* d_Output,unsigned short* d_Input)
{
	...
	for (int pos = 0 ; pos < NUMBER_HALO_BLOCK ; pos++)
	{
		if ( pixel_z >= -pos*BLOCK_SIZE_P_Z) //negative sign because we are outside block boudaries
			s_Data[threadIdx.y][threadIdx.x + pos*BLOCK_SIZE_P_Z] = __half2float(d_Input[pos*volume_offset2]) ;

		else
			s_Data[threadIdx.y][threadIdx.x + pos*BLOCK_SIZE_P_Z] = __half2float(0) ;
	}
