 ...
 */
 ...
 #ifndef _PROJECTION_SFR_XMAINDIR_KERNEL_H_
 #define _PROJECTION_SFR_XMAINDIR_KERNEL_H_

//unweighted diagonal coefficients
template <typename T>
__global__ void CoeffDiagHHT_sftr_xMainDir_kernel(T* sinogram_g)
{
		...
		for(xn_e=0;xn_e<xVolumePixelNb_GPU;xn_e++){
			...
			for(yn_e=ksie_min;yn_e<=ksie_max;yn_e++){
				...
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					...
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi_moins<=float(yVolumePixelNb_GPU-1) && ksi_plus>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*(s2-s1);
					}
				}
...
template <typename T>
__global__ void weightedCoeffDiagHVHT_sftr_xMainDir_kernel(T* sinogram_g)
{
	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	//unsigned long long int adress;
...
// projector
template <typename T>
__global__ void projection_sftr_xMainDir_kernel(T* sinogram_g)
{
	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	//unsigned long long int adress;
