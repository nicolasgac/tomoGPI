/* kernel for SF projection modelling rectangular footprint
 * both in transaxial and axial directions
 *
 * Author : Camille Chapdelaine
 * 27/07/2018
 */

 #ifndef _PROJECTION_SF_REC_REC_KERNEL_H_
 #define _PROJECTION_SF_REC_REC_KERNEL_H_

// diagonal coefficients for rays for which the source is closer to y-axis
template <typename T>
__global__ void weightedCoeffDiagHVHT_sf_rec_rec_yMainDir_kernel(T* sinogram_g)
{

	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// the source is closer to y-axis
	if((phi>=projectionNb_GPU/8 && phi<3*projectionNb_GPU/8)||(phi>=5*projectionNb_GPU/8 && phi<7*projectionNb_GPU/8)){

		// initialize
		proj=0;

		// y-axis is the primary axis
		for(yn_e=0;yn_e<yVolumePixelNb_GPU;yn_e++){
			yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]+((dxsp/dysp)*(yn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]));
			ksi=ksi+xVolumeCenterPixel_GPU;

			// neighbours in x-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>xVolumePixelNb_GPU-1){
				ksie_max=xVolumePixelNb_GPU-1;
			}
			for(xn_e=ksie_min;xn_e<=ksie_max;xn_e++){
				xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint
				// project the middle points in x-planes
				// point (xn_e-0.5,yn_e) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*betaIOcylinderC_GPU[phi])/(xphi-0.5*alphaIOcylinderC_GPU[phi]);
				// point (xn_e+0.5,yn_e) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*betaIOcylinderC_GPU[phi])/(xphi+0.5*alphaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(xVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*(s2-s1)*tex3D(volume_tex,xn_e+0.5,yn_e+0.5,zn_e+0.5);//d_volume[adresse_xne_yne_zne]
					}
				}
				f_ax=f_ax*f_tr*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*((xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))*max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));

	}

}

template <typename T>
__global__ void CoeffDiagHHT_sf_rec_rec_yMainDir_kernel(T* sinogram_g)
{

	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// the source is closer to y-axis
	if((phi>=projectionNb_GPU/8 && phi<3*projectionNb_GPU/8)||(phi>=5*projectionNb_GPU/8 && phi<7*projectionNb_GPU/8)){

		// initialize
		proj=0;

		// y-axis is the primary axis
		for(yn_e=0;yn_e<yVolumePixelNb_GPU;yn_e++){
			yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]+((dxsp/dysp)*(yn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]));
			ksi=ksi+xVolumeCenterPixel_GPU;

			// neighbours in x-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>xVolumePixelNb_GPU-1){
				ksie_max=xVolumePixelNb_GPU-1;
			}
			for(xn_e=ksie_min;xn_e<=ksie_max;xn_e++){
				xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint
				// project the middle points in x-planes
				// point (xn_e-0.5,yn_e) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*betaIOcylinderC_GPU[phi])/(xphi-0.5*alphaIOcylinderC_GPU[phi]);
				// point (xn_e+0.5,yn_e) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*betaIOcylinderC_GPU[phi])/(xphi+0.5*alphaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(xVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*(s2-s1);
					}
				}
				f_ax=f_ax*f_tr*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*((xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))*max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));

	}

}

// projector for rays for which the source is closer to y-axis
template <typename T>
__global__ void projection_sf_rec_rec_yMainDir_kernel(T* sinogram_g)
{

	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// the source is closer to y-axis
	if((phi>=projectionNb_GPU/8 && phi<3*projectionNb_GPU/8)||(phi>=5*projectionNb_GPU/8 && phi<7*projectionNb_GPU/8)){

		// initialize
		proj=0;

		// y-axis is the primary axis
		for(yn_e=0;yn_e<yVolumePixelNb_GPU;yn_e++){
			yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]+((dxsp/dysp)*(yn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]));
			ksi=ksi+xVolumeCenterPixel_GPU;

			// neighbours in x-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>xVolumePixelNb_GPU-1){
				ksie_max=xVolumePixelNb_GPU-1;
			}
			for(xn_e=ksie_min;xn_e<=ksie_max;xn_e++){
				xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint
				// project the middle points in x-planes
				// point (xn_e-0.5,yn_e) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*betaIOcylinderC_GPU[phi])/(xphi-0.5*alphaIOcylinderC_GPU[phi]);
				// point (xn_e+0.5,yn_e) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*betaIOcylinderC_GPU[phi])/(xphi+0.5*alphaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(xVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*tex3D(volume_tex,xn_e+0.5,yn_e+0.5,zn_e+0.5);//d_volume[adresse_xne_yne_zne]
					}
				}
				f_ax=f_ax*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*(sqrtf(xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*sqrtf(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));


	}

}

// diagonal coefficients for rays for which the source is closer to x-axis
template <typename T>
__global__ void weightedCoeffDiagHVHT_sf_rec_rec_xMainDir_kernel(T* sinogram_g)
{
	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	//unsigned long long int adress;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	//adress=un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU;
	//printf("%llu",adress);
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// initialize
	proj=0;

	// the source is closer to x-axis
	if(((float) phi)<(((float) projectionNb_GPU)/8)||(((float) phi)>=3*((float) projectionNb_GPU)/8 && ((float) phi)<5*((float) projectionNb_GPU)/8)||((float) phi)>=7*((float) projectionNb_GPU)/8){

		// x-axis is the primary axis
		for(xn_e=0;xn_e<xVolumePixelNb_GPU;xn_e++){
			xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]+((dysp/dxsp)*(xn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]));
			ksi=ksi+yVolumeCenterPixel_GPU;

			// neighbours in y-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>yVolumePixelNb_GPU-1){
				ksie_max=yVolumePixelNb_GPU-1;
			}
			for(yn_e=ksie_min;yn_e<=ksie_max;yn_e++){
				yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint : rectangular
				// project the middle points in y-planes
				// point (xn_e,yn_e-0.5) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*alphaIOcylinderC_GPU[phi])/(xphi-0.5*betaIOcylinderC_GPU[phi]);
				// point (xn_e,yn_e+0.5) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*alphaIOcylinderC_GPU[phi])/(xphi+0.5*betaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(yVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*(s2-s1)*tex3D(volume_tex,xn_e+0.5,yn_e+0.5,zn_e+0.5);//d_volume[adresse_xne_yne_zne]
					}
				}
				f_ax=f_ax*f_tr*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*((xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))*max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));

	}


}

template <typename T>
__global__ void CoeffDiagHHT_sf_rec_rec_xMainDir_kernel(T* sinogram_g)
{
	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	//unsigned long long int adress;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	//adress=un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU;
	//printf("%llu",adress);
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// initialize
	proj=0;

	// the source is closer to x-axis
	if(((float) phi)<(((float) projectionNb_GPU)/8)||(((float) phi)>=3*((float) projectionNb_GPU)/8 && ((float) phi)<5*((float) projectionNb_GPU)/8)||((float) phi)>=7*((float) projectionNb_GPU)/8){

		// x-axis is the primary axis
		for(xn_e=0;xn_e<xVolumePixelNb_GPU;xn_e++){
			xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]+((dysp/dxsp)*(xn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]));
			ksi=ksi+yVolumeCenterPixel_GPU;

			// neighbours in y-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>yVolumePixelNb_GPU-1){
				ksie_max=yVolumePixelNb_GPU-1;
			}
			for(yn_e=ksie_min;yn_e<=ksie_max;yn_e++){
				yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint : rectangular
				// project the middle points in y-planes
				// point (xn_e,yn_e-0.5) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*alphaIOcylinderC_GPU[phi])/(xphi-0.5*betaIOcylinderC_GPU[phi]);
				// point (xn_e,yn_e+0.5) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*alphaIOcylinderC_GPU[phi])/(xphi+0.5*betaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(yVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*(s2-s1);
					}
				}
				f_ax=f_ax*f_tr*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*((xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))*max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));

	}


}


// projector for rays for which the source is closer to x-axis
template <typename T>
__global__ void projection_sf_rec_rec_xMainDir_kernel(T* sinogram_g)
{
	// volume copié sur la mémoire de texture
	unsigned long int phi,un_e, vn_e;
	//unsigned long long int adress;
	unsigned int u_min, u_max;
	float proj;
	float xn_prime,yn_prime,zn_prime,un_prime,vn_prime;
	float ksi,ksi_moins,ksi_plus,xphi,yphi,v_moins,v_plus;
	unsigned int xn_e,yn_e,zn_e,ksie_min,ksie_max,zne_min,zne_max;
	float f_ax, f_tr,u_moins,u_plus,s1,s2;
	float tau1, tau2,dxsp,dysp;
	bool in_u;

	un_e=threadIdx.x+blockIdx.x*blockDim.x;
	vn_e=threadIdx.y+blockIdx.y*blockDim.y;//+vn_start;
	phi=threadIdx.z + blockIdx.z*blockDim.z;//+phi_start;
	//adress=un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU;
	//printf("%llu",adress);
	un_prime=((float)un_e-uDetectorCenterPixel_GPU);
	vn_prime=((float)vn_e-vDetectorCenterPixel_GPU);

	//x_p-x_s
	dxsp=focusDetectorDistance_GPU*alphaIOcylinderC_GPU[phi]-un_prime*uDetectorPixelSize_GPU*betaIOcylinderC_GPU[phi];
	//y_p-y_s
	dysp=focusDetectorDistance_GPU*betaIOcylinderC_GPU[phi]+un_prime*uDetectorPixelSize_GPU*alphaIOcylinderC_GPU[phi];

	// initialize
	proj=0;

	// the source is closer to x-axis
	if(((float) phi)<(((float) projectionNb_GPU)/8)||(((float) phi)>=3*((float) projectionNb_GPU)/8 && ((float) phi)<5*((float) projectionNb_GPU)/8)||((float) phi)>=7*((float) projectionNb_GPU)/8){

		// x-axis is the primary axis
		for(xn_e=0;xn_e<xVolumePixelNb_GPU;xn_e++){
			xn_prime=(float)xn_e-xVolumeCenterPixel_GPU;
			ksi=-(focusObjectDistance_GPU/xVolumePixelSize_GPU)*betaIOcylinderC_GPU[phi]+((dysp/dxsp)*(xn_prime+(focusObjectDistance_GPU/xVolumePixelSize_GPU)*alphaIOcylinderC_GPU[phi]));
			ksi=ksi+yVolumeCenterPixel_GPU;

			// neighbours in y-axis
			ksi_moins=ksi-0.5;
			if(ksi_moins<0){
				ksi_moins=0;
			}
			ksie_min=floor(ksi_moins);
			ksi_plus=ksi+0.5;
			ksie_max=ceil(ksi_plus);
			if(ksie_max>yVolumePixelNb_GPU-1){
				ksie_max=yVolumePixelNb_GPU-1;
			}
			for(yn_e=ksie_min;yn_e<=ksie_max;yn_e++){
				yn_prime=(float)yn_e-yVolumeCenterPixel_GPU;
				xphi=(focusObjectDistance_GPU/xVolumePixelSize_GPU)+alphaIOcylinderC_GPU[phi]*xn_prime+betaIOcylinderC_GPU[phi]*yn_prime;
				yphi=-betaIOcylinderC_GPU[phi]*xn_prime+alphaIOcylinderC_GPU[phi]*yn_prime;

				// transaxial footprint : rectangular
				// project the middle points in y-planes
				// point (xn_e,yn_e-0.5) : p1
				v_moins=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi-0.5*alphaIOcylinderC_GPU[phi])/(xphi-0.5*betaIOcylinderC_GPU[phi]);
				// point (xn_e,yn_e+0.5) : p2
				v_plus=(focusDetectorDistance_GPU/uDetectorPixelSize_GPU)*(yphi+0.5*alphaIOcylinderC_GPU[phi])/(xphi+0.5*betaIOcylinderC_GPU[phi]);
				tau1=min(v_moins,v_plus);
				tau2=max(v_moins,v_plus);
				u_moins=uDetectorCenterPixel_GPU-0.5+tau1;//-0.5
				if(u_moins<0){
					u_moins=0;
				}
				u_min=floor(u_moins);
				u_plus=uDetectorCenterPixel_GPU+0.5+tau2;//+0.5
				u_max=ceil(u_plus);
				if(u_max>uDetectorPixelNb_GPU-1){
					u_max=uDetectorPixelNb_GPU-1;
				}

				// the cell lies in the footprint
				in_u=(un_e<=u_max && un_e>=u_min && u_moins<=float(uDetectorPixelNb_GPU-1) && u_plus>=0);

				// transaxial footprint
				s1=max((un_prime-0.5),tau1);
				s2=min((un_prime+0.5),tau2);
				if(s1<s2 && in_u){
					f_tr=s2-s1;
				}else{
					f_tr=0.0;
				}

				// calcul de z pour lesquels F2 non nul
				u_moins=zVolumeCenterPixel_GPU-0.5+((vn_prime-0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//-0.5
				u_plus=zVolumeCenterPixel_GPU+0.5+((vn_prime+0.5)*xphi*vDetectorPixelSize_GPU/focusDetectorDistance_GPU);//+0.5
				if(u_moins<0){
					u_moins=0;
				}
				zne_min=floor(u_moins);
				zne_max=ceil(u_plus);
				if(zne_max>zVolumePixelNb_GPU-1){
					zne_max=zVolumePixelNb_GPU-1;
				}

				// axial footprint
				f_ax=0;
				for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
					zn_prime=((float)zn_e-zVolumeCenterPixel_GPU);
					v_moins=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime-0.5)/xphi);
					v_plus=(focusDetectorDistance_GPU/vDetectorPixelSize_GPU)*((zn_prime+0.5)/xphi);

					s1=max((vn_prime-0.5),v_moins);
					s2=min((vn_prime+0.5),v_plus);
					if(s1<s2 && u_moins<=float(zVolumePixelNb_GPU-1) && u_plus>=0 && ksi<=float(yVolumePixelNb_GPU-1) && ksi>=0 && ((xn_prime*xn_prime+yn_prime*yn_prime)<0.25*float(xVolumePixelNb_GPU)*float(xVolumePixelNb_GPU))){
						f_ax+=(s2-s1)*tex3D(volume_tex,xn_e+0.5,yn_e+0.5,zn_e+0.5);//d_volume[adresse_xne_yne_zne]
					}
				}
				f_ax=f_ax*f_tr;// footprint of the voxel

				//A2 method
				s1=xphi*alphaIOcylinderC_GPU[phi]-yphi*betaIOcylinderC_GPU[phi];//cos_psi
				s2=xphi*betaIOcylinderC_GPU[phi]+yphi*alphaIOcylinderC_GPU[phi];//sin_psi
				proj+=f_ax*(sqrtf(xphi*xphi+yphi*yphi)*xVolumePixelSize_GPU/(max(abs(s1),abs(s2))));

			}
		}

		// méthode A1 ou A2
		un_prime=un_prime*uDetectorPixelSize_GPU;
		vn_prime=vn_prime*vDetectorPixelSize_GPU;
		sinogram_g[un_e+vn_e*uDetectorPixelNb_GPU+phi*uDetectorPixelNb_GPU*vDetectorPixelNb_GPU]=proj*sqrtf(1+((vn_prime*vn_prime)/(un_prime*un_prime+focusDetectorDistance_GPU*focusDetectorDistance_GPU)));

	}


}

#endif
