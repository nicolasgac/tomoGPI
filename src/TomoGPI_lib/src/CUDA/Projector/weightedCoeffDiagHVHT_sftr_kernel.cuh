...
* August 03, 2018
*/
#ifndef _WEIGHTEDCOEFFDIAGHVHT_SFTR_KERNEL_H_
#define _WEIGHTEDCOEFFDIAGHVHT_SFTR_KERNEL_H_
// weighted diagonal coefficients
template <typename T>
__global__ void weightedCoeffDiagHVHT_sftr_generalized_kernel_v1(T* sinogram_g)
{
		...
		* *****************/
		...
		for(zn_e=zne_min;zn_e<=zne_max;zn_e++){
			...
			if(dxsp<dysp && in_u && (abs(pas_x)<1.0 || abs(pas_y)<1.0)){
				f_ax0+=(dysp-dxsp)*(dysp-dxsp)*tex3D(volume_tex,xn_e+0.5,yn_e+0.5,zn_e+0.5);//d_volume[adresse_xne_yne_zne]
			}
...
template <typename T>
__global__ void weightedCoeffDiagHVHT_sftr_generalized_kernel_v2(int K, T* sinogram_g)
{
	// volume copié sur la mémoire de texture
				...
				if ((xn_prime+1.0<=float(zVolumePixelNb_GPU) && yn_prime>=0)){
					...
					for(zn_e=v_max;zn_e<=zne_max;zn_e++){
						...
						for (;vn_e < v_max; vn_e++){
							...
							if(dysp > 0){
								f_ax0[id + BLOCK_SIZE*vn_e]+=dysp*dysp*xn_prime;//d_volume[xn_e+yn_e*xVolumePixelNb_GPU+zn_e*xVolumePixelNb_GPU*yVolumePixelNb_GPU]
							}
							vn_prime += 1.0f;
							dxsp += 1.0f;
						}
	...
	for (v_max = 0; v_max < K; v_max++){
		sinogram_g[un_e]=proj[id + BLOCK_SIZE*v_max]*(1+vn_prime*vn_prime*f_tr0);
...
template <typename T>
__global__ void weightedCoeffDiagHVHT_sftr_generalized_kernel_v2_CPU(int K, T* sinogram_g, int vn_start, int zn_start)
{
	// volume copié sur la mémoire de texture
	...
	for (v_max = 0; v_max < K; v_max++){
		sinogram_g[un_e]=(1+vn_prime*vn_prime*f_tr0)*proj[id + BLOCK_SIZE*v_max];
		vn_prime += vDetectorPixelSize_GPU;
