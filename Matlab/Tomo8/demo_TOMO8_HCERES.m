...
f_estimated=CreateVolumeInit(iter);
 doGradient(iter,f_estimated,g_real,f_real);
 
 taille=size(f_real,1)/2;
 
  figure(1);plot(f_real(:,taille,taille),'k','LineWidth',1.5);hold on;
%  figure(1);plot(f_FDK(:,taille,taille),'r','LineWidth',1.5,'Marker','o');hold on;
  figure(1);plot(f_estimated(:,taille,taille),'b','LineWidth',1.5,'Marker','+');hold on;
